...
Node.prototype.send = function(msg) {
        ...
        if (this._wire) {
            ...
            if (node) {
                if (process.env.ROLE == "worker") {
                    totalMsg = {};
                    totalMsg._nodeId = node.id;
                    if (this._flow) {
                        totalMsg._flowId = this._flow.id;
                    } else {
                        totalMsg._flowId = '';
                    }
                    totalMsg.origMsg = msg;
                    channels.return(totalMsg);
                } else {
                    node.receive(msg);
                }
            }
    ...
    for (i=0;i<sendEvents.length;i++) {
        ...
        if (process.env.ROLE == "worker") {
            totalMsg = {};
            totalMsg._nodeId = ev.n.id;
            if (this._flow) {
                totalMsg._flowId = this._flow.id;
            } else {
                totalMsg._flowId = '';
            }
            totalMsg.origMsg = ev.m;
            channels.return(totalMsg);
        } else {
            ev.n.receive(ev.m);
        }
 ...
 * 
 * RABBITMQ Changes:
 * When the msg is empty, a wire has been started. This places the initial message on a RabbitMQ queue.
 */
Node.prototype.receive = function(msg) {
    if (process.env.ROLE == "client" && this.type != "debug") {
        if (!msg) {
            msg = {};
            msg._msgid = redUtil.generateId();
        }
